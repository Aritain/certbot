...
def renew_cert(config, plugins, lineage):
    ...
    if installer is None:
        notify("new certificate deployed, reloading nginx, fullchain is {0}".format(
               lineage.fullchain), pause=False)

        result_reload = os.system('killall -HUP nginx')
        if result_reload == 0:
            logger.info("Nginx successfully reloaded.")
        else:
            logger.info("Nginx reload failed.")
    else:
